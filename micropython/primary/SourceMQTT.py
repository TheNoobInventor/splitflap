...
class SourceMQTT(Source):
    def __init__(self, display: Display, providers: Dict[str, Provider]):
        ...
        def load_mqtt_message(topic, msg):
            ...
            if (
                decoded_mqtt_message
                and decoded_mqtt_message != self.previous_mqtt_message
            ):
    ...
    #
    def display_data_to_message(
        self, display_data: Dict[str, str], physical_motor_position: [int]
    ) -> Tuple[Message, int]:
        ...
        message, interval_ms = provider.get_message(
            text, display_data, self.display, motor_pos
        )
        return self.display.virtual_to_physical(message), interval_ms

    #
    def load_message(
        self, is_stopped: bool, physical_motor_position: [int]
    ) -> Union[Message, None]:
        ...
        if (
            self.current_mqtt_message
            and self.current_mqtt_message != self.previous_mqtt_message
        ):

            message, interval = self.display_data_to_message(
                self.current_mqtt_message, physical_motor_position
            )
